...
#define PREV_BUTTON_WIDTH 50
#define PREV_BUTTON_HEIGHT 50
#define PREV_BUTTON_X 220
#define PREV_BUTTON_Y 800
#define PREV_BUTTON_HALF_WIDTH PREV_BUTTON_WIDTH / 2
#define PREV_BUTTON_HALF_HEIGHT PREV_BUTTON_HEIGHT / 2
...
PlayerInfoPage02::PlayerInfoPage02()
{
	classType = 0;
	BackButtonState = TEX_BACK_BUTTON_NOMAL;
	PrevButtonState = TEX_PREV_BUTTON2_NOMAL;
...
/* Update */
void PlayerInfoPage02::BackButtonUpdate(POINT pt)
{
...
void PlayerInfoPage02::PrevButtonUpdate(POINT pt)
{
	if (pt.x > PREV_BUTTON_X - PREV_BUTTON_HALF_WIDTH && pt.x < PREV_BUTTON_X + PREV_BUTTON_HALF_WIDTH
		&& pt.y > PREV_BUTTON_Y - PREV_BUTTON_HALF_HEIGHT && pt.y < PREV_BUTTON_Y + PREV_BUTTON_HALF_HEIGHT)
	{
		if (PrevButtonState == TEX_PREV_BUTTON2_NOMAL)
		{
			PrevButtonState = TEX_PREV_BUTTON2_MOUSEOVER;
		}
	...
	else if (PrevButtonState == TEX_PREV_BUTTON2_MOUSEOVER)
	{
		PrevButtonState = TEX_PREV_BUTTON2_NOMAL;
	}
}

void PlayerInfoPage02::ExitButtonUpdate(POINT pt)
{
...
void PlayerInfoPage02::DoExitUpdate(POINT pt)
{
...
/* Render */
void PlayerInfoPage02::BackgroundRender()
{
	...
	element = textureManager.getTexture(TEX_PLAYER_INFO_02_BACKGROUND);
...
void PlayerInfoPage02::dataRender()
{
	RECT rc;
	D3DXVECTOR3 pos;
	WCHAR text[256];
	...
	rc.top = 400;
	rc.bottom = 500;
	_stprintf_s<256>(text, _T("국어                           %3d"), gameSystem.player->getKoreanScore());
	...
	rc.top = 450;
	rc.bottom = 500;
	_stprintf_s<256>(text, _T("수학                           %3d"), gameSystem.player->getMathScore());
	font->DrawText(NULL, text, -1, &rc, DT_RIGHT,
		D3DXCOLOR(0.0f, 0.00f, 0.0f, 1.0f));

	rc.top = 500;
	rc.bottom = 550;
	_stprintf_s<256>(text, _T("사회                           %3d"), gameSystem.player->getSocietyScore());
	font->DrawText(NULL, text, -1, &rc, DT_RIGHT,
		D3DXCOLOR(0.0f, 0.00f, 0.0f, 1.0f));
	...
	rc.top = 550;
	rc.bottom = 600;
	_stprintf_s<256>(text, _T("과학                           %3d"), gameSystem.player->getScienceScore());
	font->DrawText(NULL, text, -1, &rc, DT_RIGHT,
		D3DXCOLOR(0.0f, 0.00f, 0.0f, 1.0f));
	...
	rc.top = 400;
	rc.bottom = 500;
	_stprintf_s<256>(text, _T("영어                           %3d"), gameSystem.player->getEnglishScore());
	font->DrawText(NULL, text, -1, &rc, DT_RIGHT,
		D3DXCOLOR(0.0f, 0.00f, 0.0f, 1.0f));
	...
	rc.top = 450;
	rc.bottom = 500;
	_stprintf_s<256>(text, _T("체육                           %3d"), gameSystem.player->getPTScore());
	font->DrawText(NULL, text, -1, &rc, DT_RIGHT,
		D3DXCOLOR(0.0f, 0.00f, 0.0f, 1.0f));

	rc.top = 500;
	rc.bottom = 550;
	_stprintf_s<256>(text, _T("기가                           %3d"), gameSystem.player->getTechnologyScore());
	font->DrawText(NULL, text, -1, &rc, DT_RIGHT,
		D3DXCOLOR(0.0f, 0.00f, 0.0f, 1.0f));
	...
	rc.top = 550;
	rc.bottom = 600;
	_stprintf_s<256>(text, _T("역사                           %3d"), gameSystem.player->getHistoryScore());
	font->DrawText(NULL, text, -1, &rc, DT_RIGHT,
		D3DXCOLOR(0.0f, 0.00f, 0.0f, 1.0f));
...
void PlayerInfoPage02::BackButtonRender()
{
	TextureElement* element;
	RECT rc;
	D3DXVECTOR3 pos;
...
void PlayerInfoPage02::PrevButtonRender()
{
	TextureElement* element;
	RECT rc;
	D3DXVECTOR3 pos;
	...
	element = textureManager.getTexture(PrevButtonState);
	element->sprite->Begin(D3DXSPRITE_ALPHABLEND);

	rc.left = 0;
	rc.top = 0;
	rc.right = PREV_BUTTON_WIDTH;
	rc.bottom = PREV_BUTTON_HEIGHT;

	pos = { PREV_BUTTON_X, PREV_BUTTON_Y, 0 };
	cen = { PREV_BUTTON_HALF_WIDTH, PREV_BUTTON_HALF_HEIGHT, 0 };
...
void PlayerInfoPage02::ExitButtonRender()
{
	TextureElement* element;
	RECT rc;
	D3DXVECTOR3 pos;
...
void PlayerInfoPage02::DoExitRender()
{
	TextureElement* element;
	RECT rc;
	D3DXVECTOR3 pos;
