...
SchoolEnglishQuizPage::SchoolEnglishQuizPage()
{
	classType = TITLE_PAGE;

	quizs = new KoreanQuizs();
	dataManager.LoadEnglishQuizData(quizs);
	quizCount = 0;
...
void SchoolEnglishQuizPage::Update()
{
	POINT pt;
	GetCursorPos(&pt);
	...
	if (timer <= 0 && X_Count <= 0 && O_Count <= 0)
	{
			...
			&& pt.y > OK_BUTTON_Y - OK_BUTTON_HALF_HEIGHT && pt.y < OK_BUTTON_Y + OK_BUTTON_HALF_HEIGHT)
		{
				...
				&& inputManager.keyBuffer[VK_LBUTTON] == 0)
			{
				...
				{
					gameSystem.player->addEnglishScore(10);
				}
				else if (rightAnswerCount == wrongAnswerCount)
				{
					gameSystem.player->addEnglishScore(5);
				}
				else if (rightAnswerCount < wrongAnswerCount)
				{
					gameSystem.player->addEnglishScore(0);
				}
...
void SchoolEnglishQuizPage::Render()
{
	BackgroundRender();
...
/* Render */
void SchoolEnglishQuizPage::BackgroundRender()
{
	TextureElement* element;
	RECT rc;
	D3DXVECTOR3 pos;
	...
	element = textureManager.getTexture(TEX_ENGLISH_QUIZ_PAGE_BACKGROUND);
	element->sprite->Begin(D3DXSPRITE_ALPHABLEND);
