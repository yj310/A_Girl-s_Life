...
SchoolHistoryQuizPage::SchoolHistoryQuizPage()
{
	classType = TITLE_PAGE;

	quizs = new KoreanQuizs();
	dataManager.LoadHistoryQuizData(quizs);
	quizCount = 0;
...
void SchoolHistoryQuizPage::Update()
{
	POINT pt;
	GetCursorPos(&pt);
	...
	if (timer <= 0 && X_Count <= 0 && O_Count <= 0)
	{
			...
			&& pt.y > OK_BUTTON_Y - OK_BUTTON_HALF_HEIGHT && pt.y < OK_BUTTON_Y + OK_BUTTON_HALF_HEIGHT)
		{
				...
				&& inputManager.keyBuffer[VK_LBUTTON] == 0)
			{
				...
				{
					gameSystem.player->addHistoryScore(10);
				}
				else if (rightAnswerCount == wrongAnswerCount)
				{
					gameSystem.player->addHistoryScore(5);
				}
				else if (rightAnswerCount < wrongAnswerCount)
				{
					gameSystem.player->addHistoryScore(0);
				}
...
void SchoolHistoryQuizPage::Render()
{
	BackgroundRender();
...
/* Render */
void SchoolHistoryQuizPage::BackgroundRender()
{
	TextureElement* element;
	RECT rc;
	D3DXVECTOR3 pos;
	...
	element = textureManager.getTexture(TEX_HISTORY_QUIZ_PAGE_BACKGROUND);
	element->sprite->Begin(D3DXSPRITE_ALPHABLEND);
